import numpy as np
from spin_stadium_generation import bounce
...
def main():
    ...
    LCN_vs_gamma(10, 10, 10)


def LCN_vs_gamma(N_gammas, N_starts, N_iters, MI_coeff=0.5):
    ...
    # MI_coeffic: alpha, the moment of inertia coefficient
    G = 1  # normalized area of stadium
    a_values = np.linspace(0, 1.7, N_gammas)
    # r as a function of a and g
    ...
    for i in range(len(LCNs)):
        LCNs[i] = random_starts_LCN(
            N_starts, N_iters, a=a_values[i], r=r_values[i], MI_coeff=MI_coeff)
    plt.plot(gamma_values, LCNs)
    plt.title(r'Spinnable stadium billiard - LCN as function of $\gamma$ ratio')
    plt.xlabel(r'$\gamma$ ($ = a/r$, with area normalized)')
    plt.ylabel(r'LCN $\lambda$')


def random_starts_LCN(N_starts, N_iters, a=1, r=1, MI_coeff=0.5):
    ...
    # and random velocity - normalized to one
    # start with u = 1
    for n in range(N_starts):
        ...
        x = np.array([x, y, np.cos(alpha), np.sin(alpha), 1])
        x_array.append(x)

    LCN_array = []

    for x in x_array:
        LCN = calc_LCN(x, N_iters, a=a, r=r, MI_coeff=MI_coeff)
        LCN_array.append(LCN)

    return np.mean(LCN_array)


def calc_LCN(x, N_iters, a=1, r=1, MI_coeff=0.5):
    # x is [x0, y0, vx0, vy0, u0], as numpy array
    # generate random dx
    dx = np.random.rand(2) - 0.5
    # dtheta = np.random.rand() * 2 * np.pi
    # normalize to make perturbation to specified small size
    # note: only position
    dx = np.array([dx[0], dx[1], 0, 0, 0])
    dx = dx * e_mag / np.linalg.norm(dx)
    y = x + dx  # perturbed endpoint
    ...
    for n in range(N_iters):
        ...
        x, y, t_x, t_y = bounce_endpoints(x, y, a=a, r=r, MI_coeff=MI_coeff)
...
def bounce_endpoints(x, y, a=1, r=1, MI_coeff=0.5):
    # evolve the endpoints one collision each
    x, t_x = bounce(x, a=a, r=r, MI_coeff=MI_coeff)
    y, t_y = bounce(y, a=a, r=r,  MI_coeff=MI_coeff)
